...
namespace Siya\CustomPostTypes\BlueprintPost;

class Setup {

    public function __construct() {
        add_action('init', array($this, 'create_blueprint_post_type'));
        add_action('init', array($this, 'register_blueprint_taxonomies'));
        ...
        add_filter('bulk_actions-edit-blueprint', array($this, 'remove_bulk_actions'));
        add_filter('display_post_states', array($this, 'remove_post_states'), 10, 2);
        add_filter('manage_blueprint_posts_columns', array($this, 'customize_columns'));
        ...
        // Add new filters
        add_action('restrict_manage_posts', array($this, 'add_blueprint_filters'));
        add_filter('pre_get_posts', array($this, 'filter_blueprints_by_taxonomy'));

        // Add new actions and filters
        add_filter('post_row_actions', array($this, 'modify_blueprint_actions'), 10, 2);
        add_action('init', array($this, 'modify_blueprint_capabilities'));
    ...
    /**
     * Registers a custom post type for blueprints.
     */
    public function create_blueprint_post_type() {
        $labels = array(
            'name'               => _x('Blueprints', 'post type general name', 'your-text-domain'),
            'singular_name'      => _x('Blueprint', 'post type singular name', 'your-text-domain'),
            'menu_name'          => _x('Blueprints', 'admin menu', 'your-text-domain'),
            'add_new'            => _x('Add New', 'blueprint', 'your-text-domain'),
            'add_new_item'       => __('Add New Blueprint', 'your-text-domain'),
            'edit_item'          => __('Edit Blueprint', 'your-text-domain'),
            'view_item'          => __('View Blueprint', 'your-text-domain'),
            'all_items'          => __('All Blueprints', 'your-text-domain'),
            'search_items'       => __('Search Blueprints', 'your-text-domain'),
            'not_found'          => __('No blueprints found.', 'your-text-domain'),
            'not_found_in_trash' => __('No blueprints found in Trash.', 'your-text-domain')
        );
        ...
        $args = array(
            ...
            'show_in_menu'       => true,
            'query_var'          => true,
            'rewrite'            => array('slug' => 'blueprint', 'with_front' => false),
    ...
    public function register_blueprint_taxonomies() {
        // Hierarchical taxonomy: Group
        register_taxonomy(
            'arsol_blueprint_group',
            'blueprint',
            array(
                'labels' => array(
                    'name' => __('Blueprint Groups', 'your-text-domain'),
                    'singular_name' => __('Blueprint Group', 'your-text-domain'),
                ),
                'hierarchical' => true,
                'public' => false,
                'show_ui' => true,
                'rewrite' => array('slug' => 'blueprint-group'),
            )
        );

        // Non-hierarchical taxonomy: Blueprint Tags
        register_taxonomy(
            'arsol_blueprint_tag', 
            'blueprint', 
            array(
                'labels' => array(
                    'name' => __('Blueprint Tags', 'your-text-domain'),
                    'singular_name' => __('Blueprint Tag', 'your-text-domain'),
                ),
                'hierarchical' => false,
                'public' => false,
                'show_ui' => true,
                'rewrite' => array('slug' => 'blueprint-tag'),
            )
        );
    }
    ...
    public function remove_post_table_actions($actions, $post) {
        if ($post->post_type == 'blueprint') {
            ...
            $subscription_id = get_post_meta($post->ID, 'arsol_blueprint_subscription_id', true);
            $subscription = $subscription_id ? wcs_get_subscription($subscription_id) : false;
            if (current_user_can('administrator') && get_option('arsol_allow_admin_blueprint_deletion', false) && (!$subscription_id || !$subscription)) {
    ...
    public function remove_add_new_button() {
        global $submenu;
        if (isset($submenu['edit.php?post_type=blueprint'])) {
            unset($submenu['edit.php?post_type=blueprint'][10]); // Removes 'Add New'
        }
    }
    ...
    public function restrict_capabilities($caps, $cap, $user_id, $args) {
        if (in_array($cap, ['delete_post', 'create_posts', 'delete_posts'])) {
            if (isset($args[0])) {
                ...
                if ($post && $post->post_type === 'blueprint') {
                    // Allow delete capability for administrators if the option is enabled and no valid subscription is associated
                    $subscription_id = get_post_meta($post->ID, 'arsol_blueprint_subscription_id', true);
                    $subscription = $subscription_id ? wcs_get_subscription($subscription_id) : false;
                    if ($cap === 'delete_post' && current_user_can('administrator') && get_option('arsol_allow_admin_blueprint_deletion', false) && (!$subscription_id || !$subscription)) {
                        return $caps;
                    }
                    $caps[] = 'do_not_allow';
                }
    ...
    public function change_published_to_provisioned($translated_text, $text, $domain) {
        global $post, $pagenow;
        if ($domain === 'default' && $text === 'Published' && isset($post) && $post->post_type === 'blueprint' && ($pagenow === 'edit.php' || $pagenow === 'post.php')) {
    ...
    public function remove_permalink_editor($html, $post_id, $new_title, $new_slug) {
        ...
        // Check if the post type is 'blueprint'
        if ($post->post_type === 'blueprint') {
    ...
    public function display_custom_title($post) {
        if ($post->post_type === 'blueprint') {
            echo '<div id="order_data"><h2> Blueprint: ' . esc_html($post->post_title) . '</h2></div>';
        }
    }
    ...
    public function add_blueprint_filters() {
        global $typenow;
    
        if ($typenow === 'blueprint') {
            // Check if there is at least one post of the 'blueprint' post type
            $blueprint_count = wp_count_posts('blueprint')->publish;
            if ($blueprint_count > 0) {
                // Dropdown for Blueprint Groups
                $this->render_taxonomy_dropdown(
                    'arsol_blueprint_group',
                    __('Filter by Blueprint Group', 'your-text-domain'),
                    __('All Blueprint Groups', 'your-text-domain')
                );
    
                // Dropdown for Blueprint Tags
                $this->render_taxonomy_dropdown(
                    'arsol_blueprint_tag',
                    __('Filter by Blueprint Tags', 'your-text-domain'),
                    __('All Blueprint Tags', 'your-text-domain')
                );
            }
        }
    }
    ...
    public function filter_blueprints_by_taxonomy($query) {
        global $pagenow;
    
        if ($pagenow === 'edit.php' && isset($query->query_vars['post_type']) && $query->query_vars['post_type'] === 'blueprint') {
            $tax_query = array();
    
            if (!empty($_GET['arsol_blueprint_group'])) {
                $tax_query[] = array(
                    'taxonomy' => 'arsol_blueprint_group',
                    'field'    => 'slug',
                    'terms'    => sanitize_text_field($_GET['arsol_blueprint_group']),
                );
            }
    
            if (!empty($_GET['arsol_blueprint_tag'])) {
                $tax_query[] = array(
                    'taxonomy' => 'arsol_blueprint_tag',
                    'field'    => 'slug',
                    'terms'    => sanitize_text_field($_GET['arsol_blueprint_tag']),
                );
            }
    ...
    public function modify_blueprint_actions($actions, $post) {
        if ($post->post_type === 'blueprint') {
    ...
    public function modify_blueprint_capabilities() {
        if (current_user_can('administrator') && get_option('arsol_allow_admin_blueprint_deletion', false)) {
            $role = get_role('administrator');
            $role->add_cap('delete_blueprints');
            $role->add_cap('delete_published_blueprints');
        }
    }

    public function my_column_width() {
        echo '<style type="text/css">
                .column-arsol-blueprint-status .column-details { width: 400px !important; overflow: hidden; }
              </style>';
    }

    /**
     * Load WooCommerce and WooCommerce Subscriptions styles on the blueprints table page.
     */
    public function load_woocommerce_styles($hook) {
        global $typenow;
        if ($typenow === 'blueprint' && $hook === 'edit.php') {
