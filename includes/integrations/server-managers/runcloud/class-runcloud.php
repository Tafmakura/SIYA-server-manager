...
class Runcloud /*implements ServerManager*/ {
    ...
    public function connect_server_manager_to_provisioned_server($server_post_id) {
        ...
        try {
            ...
            if (empty($private_key)) {
                ...
                error_log('[SIYA Server Manager][RunCloud] SSH private key loaded successfully.');
            }
    ...
    public function open_server_ports($server_provisioned_id) {
        ...
        if (is_wp_error($response)) {
            error_log('RunCloud open ports error: ' . $response->get_error_message());
            return false;
        }

        $response_code = wp_remote_retrieve_response_code($response);
        $response_body = wp_remote_retrieve_body($response);
        error_log('RunCloud open ports response: ' . $response_body . ', Status: ' . $response_code);
