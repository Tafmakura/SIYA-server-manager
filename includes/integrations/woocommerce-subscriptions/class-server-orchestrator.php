...
class ServerOrchestrator {
    ...
    public function provision_and_deploy_server($subscription) {
        ...
        $post_id = $this->create_server_post($subscription_id, $server_name);
        $this->provision_hetzner_server($subscription, $server_name, $post_id);
        $this->deploy_to_runcloud($subscription, $server_name, $post_id);
    }
    
    private function create_server_post($subscription_id, $server_name) {
    ...
    private function provision_hetzner_server($subscription, $server_name, $post_id) {
    ...
    private function deploy_to_runcloud($subscription, $server_name, $post_id) {
        error_log(sprintf('[SIYA Server Manager] Step 5: Starting deployment to RunCloud for subscription %d', $subscription->get_id()));
        ...
        if (empty($server_name)) {
            $server_name = 'ARSOL' . $subscription->get_id();
        ...
        error_log(sprintf('[SIYA Server Manager] Step 5: Deployment to RunCloud completed for subscription %d', $subscription->get_id()));
    }
