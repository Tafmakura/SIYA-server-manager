...
jQuery(document).ready(function($) {
    function loadProviders(serverType) {
        return $.ajax({
            url: ajaxurl,
            data: {
    ...
    function loadGroups(provider, serverType) {
        return $.ajax({
            url: ajaxurl,
            data: {
    ...
    function loadPlans(provider, group, serverType) {
        return $.ajax({
            url: ajaxurl,
            data: {
    ...
    $('#arsol_server_type').on('change', function() {
        ...
        $('#arsol_server_provider_slug').empty();
        $('#arsol_server_plan_group_slug').empty();
        $('#arsol_server_plan_slug').empty();

        if (!serverType) return;

        loadProviders(serverType).then(function(providers) {
            const $provider = $('#arsol_server_provider_slug');
    ...
    $('#arsol_server_provider_slug').on('change', function() {
        ...
        const serverType = $('#arsol_server_type').val();
        ...
        loadGroups(provider, serverType).then(function(groups) {
            const $group = $('#arsol_server_plan_group_slug');
    ...
    $('#arsol_server_plan_group_slug').on('change', function() {
        ...
        const provider = $('#arsol_server_provider_slug').val();
        const serverType = $('#arsol_server_type').val();
        ...
        loadPlans(provider, group, serverType).then(function(plans) {
            const $plan = $('#arsol_server_plan_slug');
            $plan.append('<option value="">Select plan</option>');
            plans.forEach(function(plan) {
                $plan.append(`<option value="${plan.slug}">${plan.slug}</option>`);
            });
        });
    });

    // Trigger initial load if server type is already selected
    const initialServerType = $('#arsol_server_type').val();
    if (initialServerType) {
        $('#arsol_server_type').trigger('change');
    }
